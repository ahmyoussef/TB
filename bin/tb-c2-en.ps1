...
# -------------------------------------
        ...
        $path         = $main + "bin\"
        $dpath        = $main + "data\"
        $working      = $main + "working-c2"
        $archive      = $main + "archive-c2"
        $players      = $main + "config\players-c2.csv"
        Set-Location -Path $path

    Start-Sleep -Milliseconds 500
    write-host ""
    write-host ">>>>>>> Your Clan : $clanname2" -ForegroundColor white
    ...
    write-host ">> [3] : Summary Total Score (total-datas-c2)         <<" -foregroundcolor white
    ...
    {Clear-Host
        ...
        write-host "" -foregroundcolor yellow
        py .\tb.py --config '..\config\config-c2.cfg' --capture}
    ...
    elseif ($value -eq 2)
        ...
        write-host ""
        write-host ""
        py .\tb.py --config '..\config\config-c2.cfg' --process --summary}
    ...
    elseif ($value -eq 3)
        ...
        if ($totalscore -eq 'y')
            ...
            $dori = $dpath + "captured-datas-c2.txt"
            if (test-path -path $dori) {Rename-Item -Path $dori -NewName "backup-captured-datas-c2.txt"}
            # Total Summary erstellen
            $dpath = $dpath + "total-datas-c2.txt"
            Rename-Item -Path $dpath -NewName "captured-datas-c2.txt"
            py .\tb.py --config '..\config\config-c2.cfg' --process --summary
            }
            else {& ".\tb-c2-en.ps1"}
        } 
    ...
    write-host "" -foregroundcolor yellow
    py .\tb.py --config '..\config\config-c2.cfg' --calibrate}
    ...
    elseif ($value -eq 6)
        ...
        if ($day -eq 'x'){& ".\tb-c2-en.ps1"}
        else {py .\tb.py --config '..\config\config-c2.cfg' --process --summary --start_date $day --end_date $day}
        }
    ...
    elseif ($value -eq 'E' -or $value -eq 'e' -or $value -eq 'n' -or $value -eq 'N')
        {
            ...
            if ($choice -eq '1' -or $choice -eq '2')
            {
                ...
                Get-ChildItem -Path $dpath -Filter "*c2.txt" | ForEach-Object {
                $a1 += 1
                $file = $_
                $content = Get-Content -Path $file.FullName
