...
# Type your clan as example $clanname4 = 'Flying Dragons'
# see your configuration file config/config.cfg for $clanname!
    $clanname4 = 'Type your clan #4 here'
...
# -------------------------------------
        ...
        $path         = $main + "bin\"
        $dpath        = $main + "data\"
        $working      = $main + "working-c4"
        $archive      = $main + "archive-c4"
        $players      = $main + "config\players-c4.csv"
        Set-Location -Path $path

    Start-Sleep -Milliseconds 500
    write-host ""
    write-host ">>>>>>> Your Clan : $clanname4" -ForegroundColor white
    write-host ">>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<" -foregroundcolor yellow
    write-host ">> Run TB PC-App and switch to the Language: ENGLISH  <<" -foregroundcolor yellow
    write-host ">>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<" -foregroundcolor yellow
    write-host ">> [1] : Capture Chests                               <<" -foregroundcolor white
    write-host ">> [2] : Summary Score                                <<" -foregroundcolor white
    write-host ">> [3] : Summary Total Score (total-datas-c4)         <<" -foregroundcolor white
    ...
    write-host ">> will be added, until the file is removed.          <<" -foregroundcolor green
    write-host ">> After '[2] Summary' move the files from the folder <<" -foregroundcolor green
    write-host ">> 'Chest-Counter/data/' to your backup-folder. The   <<" -foregroundcolor green
    write-host ">> data folder must be empty for the new Capture File <<" -foregroundcolor green 
    write-host ">> Tip: Capture 24h. Example from 7 p.m. to 7 p.m.    <<" -foregroundcolor green
    write-host ">>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<" -foregroundcolor yellow
    write-host "`n"

    $value = $null
    $value = Read-Host "Choose [1-6] [C-E-R-S] or [b-x] "
    ...
    {Clear-Host
        ...
        write-host "" -foregroundcolor yellow
        py .\tb.py --config '..\config\config-c4.cfg' --capture}
    ...
    elseif ($value -eq 2)
       {Clear-Host
        ...
        write-host ""
        write-host ""
        py .\tb.py --config '..\config\config-c4.cfg' --process --summary}
    ...
    elseif ($value -eq 3)
        ...
        if ($totalscore -eq 'y')
            ...
            $dori = $dpath + "captured-datas-c4.txt"
            if (test-path -path $dori) {Rename-Item -Path $dori -NewName "backup-captured-datas-c4.txt"}
            # Total Summary erstellen
            $dpath = $dpath + "total-datas-c4.txt"
            Rename-Item -Path $dpath -NewName "captured-datas-c4.txt"
            py .\tb.py --config '..\config\config-c4.cfg' --process --summary
            }
            else {& ".\tb-c4-en.ps1"}
        } 
    ...
    write-host "" -foregroundcolor yellow
    py .\tb.py --config '..\config\config-c4.cfg' --calibrate}
    ...
    elseif ($value -eq 6)
        ...
        if ($day -eq 'x'){& ".\tb-c4-en.ps1"}
        else {py .\tb.py --config '..\config\config-c4.cfg' --process --summary --start_date $day --end_date $day}
        }
    ...
    elseif ($value -eq 'E' -or $value -eq 'e' -or $value -eq 'n' -or $value -eq 'N')
        {
            ...
            if ($choice -eq '1' -or $choice -eq '2')
            {
                ...
                Get-ChildItem -Path $dpath -Filter "*c4.txt" | ForEach-Object {
                $a1 += 1
                $file = $_
                $content = Get-Content -Path $file.FullName
