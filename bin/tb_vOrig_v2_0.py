...
class TBConfig(object):
    ...
    def __init__(self, config_file, clickWait):
        ...
        try:
            ...
            #self.x1              = config_kvp.get('time_x1', '')
            #self.y1              = config_kvp.get('time_y1', '')
            #self.x2              = config_kvp.get('time_x2', '')
            #self.y2              = config_kvp.get('time_y2', '')
            # --------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------------------
    def validate_capture(self, capture, count):
        ...
        # NUEVO: detección automática de patrón multilinea tipo "From:" / "Source:"------------------------------------
        if len(rows) >= 4 and any("From" in r for r in rows) and any("Source" in r for r in rows):
            # Combinar todo el bloque en una sola línea separada por espacios
            combined = " ".join(rows)
            # Extraer tres segmentos
            try:
                part1 = combined.split("From")[0].strip()  # hasta antes de 'From'
                from_part = "From" + combined.split("From")[1].split("Source")[0].strip()  # entre From y Source
                source_part = "Source" + combined.split("Source")[1].strip()  # desde Source al final
                rows = [part1, from_part, source_part]
                print(f"[INFO] Texto OCR multilinea reparado automáticamente:\n  1) {rows[0]}\n  2) {rows[1]}\n  3) {rows[2]}")
            except Exception as e:
                print(f"[WARN] No se pudo reparar multilinea OCR: {e}")
        # --------------------------------------------------------------------------------------------------------------    
        if len(rows) % 3 > 0:
